...
NOTE: this is the NAIVE BASELINE generator. Censored sites (no failure within
the year) get rul=9999, which the baseline trainer then drops for regression.
The survival version at the repo root keeps them via event/time_to_event.
"""
import numpy as np
import pandas as pd
...
def simulate_site(site_id: int) -> pd.DataFrame:
    ...
    fail_day = int(df.loc[failed[0], "day"]) if len(failed) else None
    if fail_day is not None:
        df = df[df["day"] <= fail_day].copy()             # battery replaced after
        df["rul"] = fail_day - df["day"]
    else:
        df["rul"] = 9999                                  # censored (no failure yet)
    return df


def main():
    df = pd.concat([simulate_site(i) for i in range(N_SITES)], ignore_index=True)
    df["fail_within_7d"] = ((df["rul"] <= RUL_HORIZON) & (df["rul"] >= 0)).astype(int)
    df.to_parquet("telemetry.parquet")
    n_fail = (df["rul"] <= 7).sum()
    print(f"rows={len(df):,}  sites={df.site_id.nunique()}  "
          f"failure-events={df[df.rul==0].site_id.nunique()}  "
          f"alarm-positives={n_fail:,} ({100*n_fail/len(df):.1f}%)")
